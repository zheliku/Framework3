...
namespace GIB.Auspex
{
    ...
    public class AuspexInspector : UnityEditor.Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            bool anyAuspex = _serializedProperties.Any(p => PropertyUtility.GetAttribute<IAuspexAttribute>(p) != null);
            if (!anyAuspex)
            {
        ...
        protected void DrawSerializedProperties()
        {
            ...
            foreach (var property in GetNonGroupedProperties(_serializedProperties))
            {
                ...
                {
                    AuspexEditorGUI.PropertyField_Layout(property, includeChildren: true);
                }
            }
            ...
            foreach (var group in GetGroupedProperties(_serializedProperties))
            {
                ...
                AuspexEditorGUI.BeginBoxGroup_Layout(group.Key);
                foreach (var property in visibleProperties)
                {
                    AuspexEditorGUI.PropertyField_Layout(property, includeChildren: true);
                }

                AuspexEditorGUI.EndBoxGroup_Layout();
            }
            ...
            foreach (var group in GetFoldoutProperties(_serializedProperties))
            {
                ...
                if (_foldouts[group.Key].Value)
                {
                    foreach (var property in visibleProperties)
                    {
                        AuspexEditorGUI.PropertyField_Layout(property, true);
                    }
                }
            }
        ...
        protected void DrawNonSerializedFields(bool drawHeader = false)
        {
            if (_nonSerializedFields.Any())
            {
                if (drawHeader)
                {
                    ...
                    AuspexEditorGUI.HorizontalLine(
                ...
                {
                    AuspexEditorGUI.NonSerializedField_Layout(serializedObject.targetObject, field);
                }
            }
        }
        ...
        protected void DrawNativeProperties(bool drawHeader = false)
        {
            if (_nativeProperties.Any())
            {
                ...
                {
                    AuspexEditorGUI.NativeProperty_Layout(serializedObject.targetObject, property);
                }
            }
        }
        ...
        protected void DrawButtons(bool drawHeader = false)
        {
            if (_methods.Any())
            {
                ...
                {
                    AuspexEditorGUI.Button(serializedObject.targetObject, method);
                }
            }
        }

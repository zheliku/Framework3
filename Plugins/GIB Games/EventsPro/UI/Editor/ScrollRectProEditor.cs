...
{
	[CustomEditor(typeof(ScrollRectPro), true)]
	[CanEditMultipleObjects]
	public class ScrollRectProEditor : Editor
	{
		...
		void CalculateCachedValues()
		{
			...
			if (targets.Length == 1)
			{
				Transform transform = ((ScrollRectPro)target).transform;
				...
				if (m_HorizontalScrollbar.objectReferenceValue == null || ((ScrollbarPro)m_HorizontalScrollbar.objectReferenceValue).transform.parent != transform)
					m_HScrollbarIsNotChild = true;
				if (m_VerticalScrollbar.objectReferenceValue == null || ((ScrollbarPro)m_VerticalScrollbar.objectReferenceValue).transform.parent != transform)

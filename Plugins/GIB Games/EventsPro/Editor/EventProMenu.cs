...
namespace UnityEditor.UI
{
	static class EventProMenu
	{
		...
		[MenuItem("GameObject/UI/Button Pro", false, 2031)]
		...
		[MenuItem("GameObject/UI/Toggle Pro", false, 2032)]
		...
		[MenuItem("GameObject/UI/Slider Pro", false, 2034)]
		...
		[MenuItem("GameObject/UI/Scrollbar Pro", false, 2035)]
		...
		[MenuItem("GameObject/UI/Dropdown Pro", false, 2036)]
		...
		[MenuItem("GameObject/UI/Input Field Pro", false, 2037)]
		...
		[MenuItem("GameObject/UI/Scroll View Pro", false, 2063)]
		...
		{
			GameObject buttonRoot = CreateUIElementRoot("Button Pro", s_ThickElementSize);
			...
			ButtonPro bt = buttonRoot.AddComponent<ButtonPro>();
			SetDefaultColorTransitionValues(bt);

#if TMP_PRESENT
			TMPro.TextMeshProUGUI text = childText.AddComponent<TMPro.TextMeshProUGUI>();
		...
		{
			// Set up hierarchy
			GameObject toggleRoot = CreateUIElementRoot("Toggle Pro", s_ThinElementSize);
			...
			// Set up components
			TogglePro toggle = toggleRoot.AddComponent<TogglePro>();
			toggle.isOn = true;
...
#if TMP_PRESENT
			TextMeshProUGUI label = childLabel.AddComponent<TextMeshProUGUI>();
			label.text = "Toggle Pro";
#else
		...
		private static GameObject CreateSlider(DefaultControls.Resources resources)
		{
			...
			GameObject root = CreateUIElementRoot("Slider Pro", s_ThinElementSize);
			...
			SliderPro slider = root.AddComponent<SliderPro>();
		...
		{
			// Create GOs Hierarchy
			GameObject scrollbarRoot = CreateUIElementRoot("Scrollbar Pro", s_ThinElementSize);
			...
			ScrollbarPro scrollbar = scrollbarRoot.AddComponent<ScrollbarPro>();
		...
		{
			GameObject root = CreateUIElementRoot("Dropdown Pro", s_ThickElementSize);
			...
			ScrollbarPro scrollbarScrollbar = scrollbar.GetComponent<ScrollbarPro>();
			scrollbarScrollbar.SetDirection(ScrollbarPro.Direction.BottomToTop, true);
			...
			TogglePro itemToggle = item.AddComponent<TogglePro>();
			...
			ScrollRectPro templateScrollRect = template.AddComponent<ScrollRectPro>();
			...
			Text labelText = itemLabel.AddComponent<Text>();
			...
			DropdownPro dropdown = root.AddComponent<DropdownPro>();
		...
		{
			GameObject root = CreateUIElementRoot("InputField Pro", s_ThickElementSize);
			...
			InputFieldPro inputField = root.AddComponent<InputFieldPro>();
		...
		{
			GameObject root = CreateUIElementRoot("Scroll View Pro", new Vector2(200, 200));
			...
			vScrollbar.GetComponent<ScrollbarPro>().SetDirection(ScrollbarPro.Direction.BottomToTop, true);
			...
			ScrollRectPro scrollRect = root.AddComponent<ScrollRectPro>();
			scrollRect.content = contentRT;
			scrollRect.viewport = viewportRT;
			scrollRect.horizontalScrollbar = hScrollbar.GetComponent<ScrollbarPro>();
			scrollRect.verticalScrollbar = vScrollbar.GetComponent<ScrollbarPro>();
		...
		private static void SetDefaultTextValues(Text lbl)
		{
			...
			// Known error: This may cause errors for some 2020 beta versions.
...
#if TMP_PRESENT
		private static void SetDefaultTextValues(TMPro.TextMeshProUGUI lbl)
		{
			// Set text values we want across UI elements in default controls.
			// Don't set values which are the same as the default values for the Text component,
			// since there's no point in that, and it's good to keep them as consistent as possible.

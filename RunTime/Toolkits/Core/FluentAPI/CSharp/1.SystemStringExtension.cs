...
namespace Framework3.Toolkits.FluentAPI
{
    ...
    public static class SystemStringExtension
    {
        ...
        {
            var strs = self.Split(delimiter, StringSplitOptions.RemoveEmptyEntries);

            if (strs.Length == 0) return Array.Empty<float>(); // 判空

            return Array.ConvertAll(strs, float.Parse); // 转换 float
        }
        ...
        public static int[] SplitToInt(this string self, char delimiter)
        {
            ...
            return Array.ConvertAll(strs, int.Parse); // 转换 int
        }
        ...
        /// </code>
        /// </example>
        public static DateTime ToDateTime(this string self, DateTime defaultValue = default)
        {

...
namespace Framework3.Toolkits.DataKit
{
    ...
    public class TxtHelper
    {
    ...
    #if UNITY_EDITOR
        [MenuItem("Framework3/DataKit/Open Txt Folder")]
        public static void OpenTxtSavePath()
        {
            DataKit.TxtSavePath.EnsureDirectoryExist();
            EditorUtility.RevealInFinder(DataKit.TxtSavePath.ConvertToCurrentEnvironmentPath());
        }
    #endif
        ...
        {
            var fullPath = filePath.ChangeExtension(extension) // 确保文件路径扩展名为指定格式
               .EnsureDirectoryExist();                        // 确保文件所在目录存在

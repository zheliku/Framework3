...
namespace Framework3.Toolkits.UIKit
{
    ...
    public class UI3DKit
    {
        ...
        {
            var handle = LoadPanelAsync<T>(panelName, null);
        ...
        {
            var handle = ShowPanelAsync<T>(panelName, null);
            return handle.WaitForCompletion().GetComponent<T>();
        }
        ...
        {
            UI3DMgr.Instance.HidePanel<T>(panelName, callback);
        }
